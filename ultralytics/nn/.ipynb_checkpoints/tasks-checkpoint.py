...
# # Custom Model for Video Detection
# class VideoDetectionModel(DetectionModel):
#     def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True, sequence_length=4):
#         """
#         Initialize the VideoDetectionModel for processing image sequences.

#         Args:
#             cfg (str): Model configuration file or dictionary. Defaults to "yolo11n.yaml".
#             ch (int): Number of channels per frame (e.g., 3 for RGB). Defaults to 3.
#             nc (int, optional): Number of classes. If None, determined from the config. Defaults to None.
#             verbose (bool): Whether to print model information during initialization. Defaults to True.
#             sequence_length (int): Number of frames per sequence. Defaults to 4.
#         """
#         # Adjust input channels to account for sequence length (Seq_Len * C)
#         super().__init__(cfg=cfg, ch=ch * sequence_length, nc=nc, verbose=verbose)
#         self.sequence_length = sequence_length  # Store the number of frames in each sequence
#         self.channels_per_frame = ch  # Store channels per frame (e.g., 3 for RGB)

#     def predict(self, x, profile=False, visualize=False, embed=None):
#         """
#         x: (B, T*C, H, W).

#         1) Pass x through patch_embed + pos_embed => shape (B, T, H', W', dim).
#         2) For t in [0..T-1], slice out (B, H', W', dim), flatten => (B, H'*W', dim).
#         3) Pass to modules [2..], do detection or hidden-state logic.
#         4) Collect detections => (B, T, #boxes, 6).
#         """

#         # The YOLO code typically uses y[] for skip connections, but let's keep it simple
#         # If you need them, you can adapt accordingly.

#         # Typically, your model list: [patch_embed, pos_embed, ViLBlockPair, ... , Detect]
#         # We'll assume the first two are patch + pos embed
#         patch_embed_mod = self.model[0]
#         pos_embed_mod   = self.model[1]

#         # => (B, T, H', W', dim)
#         if profile:
#             self._profile_one_layer(patch_embed_mod, x, [])
#         x = patch_embed_mod(x)

#         # => still (B, T, H', W', dim)
#         if profile:
#             self._profile_one_layer(pos_embed_mod, x, [])
#         x = pos_embed_mod(x)

#         B, T, Hp, Wp, dim = x.shape
#         all_detections = []
#         hidden_states = {}  # {module_idx: ...} if we store any states
...
#             # Flatten => (B, Hp*Wp, dim)
#             frame_data = einops.rearrange(frame_4d, "b h w d -> b (h w) d")

#             # Now pass frame_data to modules [2..]
#             for m_idx in range(2, len(self.model)):
#                 m = self.model[m_idx]

#                 # If YOLO skip logic is needed, you'd do something with `m.f` or `m.i` here

#                 # If profile:
#                 #    self._profile_one_layer(m, frame_data, dt=...)

#                 # Detect check
#                 if getattr(m, "type", "") == "Detect" or hasattr(m, "detect"):
#                     # final detection for this frame
#                     frame_dets = m(frame_data)
#                     all_detections.append(frame_dets)
#                     break

#                 # If it's a recurrent block
#                 elif getattr(m, "use_hidden_states", False):
#                     old_state = hidden_states.get(m_idx, None)
#                     if old_state is None:
#                         out = m(frame_data)
#                     else:
#                         out = m(frame_data, *old_state)

#                     if isinstance(out, tuple):
#                         # typical out: (frame_data, new_state)
#                         frame_data, new_state = out
#                         hidden_states[m_idx] = new_state
#                     else:
#                         frame_data = out

#                 else:
#                     # normal forward
#                     frame_data = m(frame_data)

#         # stack => (B, T, #boxes, 6)
#         if all_detections:
#             out = torch.stack(all_detections, dim=1)
#         else:
#             out = None
...
#         Args:
#             batch (dict): Batch dictionary containing 'img', 'cls', 'bboxes', 'batch_idx', etc.
#             preds (list, optional): Precomputed predictions. If None, computed internally.
